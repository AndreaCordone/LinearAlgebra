...
Matrix Q = SpecialMatrix::DiagMatrix(n,m,1.0); 
...
for (size_t k = 0; k < n-1; ++k){
  ...
  auto [MaxPivotRowIdx, MaxPivotColIdx] = MatrixOperation::MaxAbsoluteElement(Ak.Slice(k,n-1,k,m-1)); 
  ...
  Matrix Qk (n,m,0.0);
  Qk = MatrixOperation::ColPivotMatrix(A,k,MaxPivotColIdx+k); 

  Matrix PAk = MatrixArithmetic::MatrixMultiplication(Pk,Ak); 

  Matrix PAQk = MatrixArithmetic::MatrixMultiplication(PAk,Qk); 


  // Gauss Matrix Computation on the pivoted matrix PAQk
  for (size_t Idx = k+1; Idx < n; ++Idx){
    Mk(Idx, k) = - PAQk(Idx,k)/ PAQk(k,k); 
  }
  ...
  Ak = MatrixArithmetic::MatrixMultiplication(Mk,PAQk); 
  P = MatrixArithmetic::MatrixMultiplication(Pk,P); 
  Q = MatrixArithmetic::MatrixMultiplication(Q,Qk); 
...
std::vector<Matrix> LUPQ = {L,U,P,Q}; 
